...
class DragonKT(screen: GameScreen, x: Float, y: Float, internal var rayHandler: RayHandler) : BadGuy(screen, x, y) {
    var currentState: DragonKT.State
    var previousState: DragonKT.State
    val atlas: TextureAtlas = TextureAtlas("dragoon.atlas")
    ...
    val state: State
        get() = if (b2body.linearVelocity.y > 0) {
            ...
            DragonKT.State.RIGHT
        } else if (b2body.linearVelocity.x < 0) {
            DragonKT.State.LEFT
        } else if (b2body.linearVelocity.y < 0) {
            DragonKT.State.DOWN
        } else {
            DragonKT.State.STANDING
        }
    ...
    init {
        ...
        currentState = DragonKT.State.STANDING
        previousState = DragonKT.State.STANDING
        stateTimer = 0f
        runningRight = true
    ...
    fun getFrame(dt: Float): TextureRegion {
        ...
        when (currentState) {
            DragonKT.State.UP -> region = baddieUp.getKeyFrame(stateTimer, true) as TextureRegion

            DragonKT.State.LEFT -> region = baddieLeft.getKeyFrame(stateTimer, true) as TextureRegion

            DragonKT.State.RIGHT -> region = baddieRight.getKeyFrame(stateTimer, true) as TextureRegion

            DragonKT.State.DOWN -> region = baddieDown.getKeyFrame(stateTimer, true) as TextureRegion
    ...
    override fun defineBadGuy(x: Float, y: Float) {
        ...
        fdef.shape = shape
        b2body.createFixture(fdef).userData = "DragonKT"
